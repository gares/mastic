Coverage
  ...
  * ERROR: stack [fun; x; (; x]
    ...
    RECOVERY: push (squashed) (Parser.ERROR_TOKEN [('x',8,9); (':',10,11)]) on [fun; x; (]
  SHIFT [fun; x; (; perr]
  ...
  * ERROR: stack [fun; x; (; (Ast.Cmd.Err [('x',8,9); (':',10,11)]); ;; (Ast.Cmd.Err [('_',13,13)])]
    LOOKAHEAD: Parser.ELSE (out of place token)
      PROPOSE: reductions: [<list cmd> := <cmd>]
      PROPOSE: tokens: 
    RECOVERY: reduce [<list cmd> := <cmd>]
  READ Parser.ELSE
  RED 3 [fun; x; (; (Ast.Cmd.Err [('x',8,9); (':',10,11)]); ;; [(Ast.Cmd.Err [('_',13,13)])]]
  * ERROR: stack [fun; x; (; [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])]]
    LOOKAHEAD: Parser.ELSE (out of place token)
      PROPOSE: reductions: 
      PROPOSE: tokens: )
    RECOVERY: generate ) and push (generation_streak = 1)
  SHIFT [fun; x; (; [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])]; )]
  READ Parser.ELSE
  RED 5 [fun; x; (; [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])]; )]
  * ERROR: stack [(Ast.Func.Fun ("x",
  ...
  SHIFT [(Ast.Func.Fun ("x",
     [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])])); perr]
  READ (Parser.IDENT "x")
  RED 1 [(Ast.Func.Fun ("x",
     [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])])); perr]
  * ERROR: stack [(Ast.Func.Fun ("x",
     [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])])); (Ast.Func.Err [('else',13,17)])]
    LOOKAHEAD: (Parser.IDENT "x") (out of place token)
      PROPOSE: reductions: 
      PROPOSE: tokens: 
    RECOVERY: turn (Parser.IDENT "x") into (Parser.ERROR_TOKEN [('x',18,19)]) and push
  SHIFT [(Ast.Func.Fun ("x",
     [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])])); (Ast.Func.Err [('else',13,17)]); perr]
  READ Parser.RPAREN
  RED 1 [(Ast.Func.Fun ("x",
     [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])])); (Ast.Func.Err [('else',13,17)]); perr]
  * ERROR: stack [(Ast.Func.Fun ("x",
     [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])])); (Ast.Func.Err [('else',13,17)]); (Ast.Func.Err [('x',18,19)])]
    LOOKAHEAD: Parser.RPAREN (out of place token)
      PROPOSE: reductions: 
      PROPOSE: tokens: 
    RECOVERY: turn Parser.RPAREN into (Parser.ERROR_TOKEN [(')',20,21)]) and push
  SHIFT [(Ast.Func.Fun ("x",
     [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])])); (Ast.Func.Err [('else',13,17)]); (Ast.Func.Err [('x',18,19)]); perr]
  READ Parser.EOF
  RED 1 [(Ast.Func.Fun ("x",
     [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])])); (Ast.Func.Err [('else',13,17)]); (Ast.Func.Err [('x',18,19)]); perr]
  RED 0 [(Ast.Func.Fun ("x",
     [(Ast.Cmd.Err [('x',8,9); (':',10,11)]); (Ast.Cmd.Err [('_',13,13)])])); (Ast.Func.Err [('else',13,17)]); (Ast.Func.Err [('x',18,19)]); (Ast.Func.Err [(')',20,21)])]
